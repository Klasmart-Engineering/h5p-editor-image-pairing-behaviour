...
export default class ImageSequencingBehaviour {
  ...
  initFieldMaxColumns() {
    ...
    this.fieldMaxColumns.field.max = Math.max(
      ...
      (this.listCards.getValue() || []).length
    );
  }
  ...
  findCards(parent) {
    ...
    const list = parent.children.filter(child => {
      return (
        ...
        child.getId().indexOf('field-cards') === 0
      );
    });
  ...
  handleParentReady() {
    ...
    this.listCards = this.findCards(this.parent);
    ...
    if (this.fieldEnforceColumns && this.fieldMaxColumns && this.listCards) {
      this.initFieldMaxColumns();

      this.listCards.on('addedItem', () => {
        this.handlelistCardsChanged();
        this.handleFieldMaxColumnsChanged();
      });
      this.listCards.on('removedItem', () => {
  ...
  handleFieldEnforceColumnsChanged(checked) {
    ...
    this.fieldMaxColumns.$errors.html(
      `<p>${H5PEditor.t('H5PEditor.ImagePairBehaviour', 'maxColumnsRequired')}</p>`
    );
  }
  ...
  handleFieldMaxColumnsChanged() {
    if (this.fieldMaxColumns.$input.val() > this.fieldMaxColumns.field.max) {
      ...
      this.fieldMaxColumns.$errors.html(
        `<p>${H5PEditor.t('H5PEditor.ImagePairBehaviour', 'maxColumnsExceeded')}</p>`
      );
    }
  }
}
